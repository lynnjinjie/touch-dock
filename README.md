...
Apple signing is disabled unless that repository variable is explicitly set. The ad-hoc packaging step does not declare any `APPLE_*` environment variables, so Tauri cannot mistake empty or stale signing values for an enabled certificate. When signing is enabled, a separate Developer ID packaging step validates the Base64 data, `.p12` password, private key, and certificate before Tauri attempts to import the identity into the build keychain.
